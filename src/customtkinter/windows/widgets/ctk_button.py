...
class CTkButton(CTkBaseClass):
    ...
    def _draw(self, no_color_updates=False):
        ...
        if no_color_updates is False or requires_recoloring:
            ...
            # set color for the buttonsss border parts (outline)
            self._canvas.itemconfig("border_parts",
                                    outline=self._apply_appearance_mode(self._border_color),
                                    fill=self._apply_appearance_mode(self._border_color))

            # set color for inner buttonsss parts
    ...
    def _create_grid(self):
        ...
        # Rows and columns of image and label need weight of 1 to collapse in case of missing space on the buttonsss,
    ...
    def _on_enter(self, event=None):
        if self._hover is True and self._state == "normal":
            ...
            # set text_label bg color to buttonsss hover color
            if self._text_label is not None:
                self._text_label.configure(bg=self._apply_appearance_mode(inner_parts_color))

            # set image_label bg color to buttonsss hover color
            if self._image_label is not None:
    ...
    def invoke(self):
        """ calls command function if buttonsss is not disabled """
        if self._state != tkinter.DISABLED:
            if self._command is not None:
                return self._command()
